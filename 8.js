...
const decryptMessage = (symbols, shift, encodedSymbols) => {
	let decodedMessage = [];
	encodedSymbols.map((item) => {
		if (item + shift > symbols.length) {
			decodedMessage.push(symbols[item + shift - array.length]);
		} else {
			decodedMessage.push(symbols[item + shift]);
		}
	});
	return decodedMessage.join("");
};

console.log(decryptMessage(symbols, shift, encodedSymbols));
