...
const createLuggage = (startIndex, quantity, luggage) => {
	let chosenLuggage = [];
	luggage.map((item, index) => {
		if (index >= startIndex && index < startIndex + quantity) {
			chosenLuggage.push(item);
		}
	});
	return chosenLuggage;
};

console.log(createLuggage(startIndex, quantity, luggage));
